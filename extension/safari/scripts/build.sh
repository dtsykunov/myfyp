...
repo_root="$(cd "${safari_dir}/../.." && pwd)"
manifest_path="${safari_dir}/manifest.json"
...
if command -v python3 >/dev/null 2>&1; then
  ...
  echo "python3 (or python) is required to build the Safari extension package." >&2
  exit 1
fi

if ! command -v zip >/dev/null 2>&1; then
  echo "zip is required to build the Safari extension package." >&2
  exit 1
fi
...
mkdir -p "${out_dir}"
zip_path="${out_dir}/myfyp-safari-v${version}.zip"
...
echo "Built Safari extension package: ${zip_path}"
